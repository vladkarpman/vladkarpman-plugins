...
```yaml
# Theme for device testing (wraps preview in your app theme)
theme:
...
component_library:
  ...
  cards:
    - name: "ContentCard"
      ...
      use_when: "content container, elevated surface"
      signature: "(modifier: Modifier, content: @Composable () -> Unit)"
```

The `component_library` config enables the design-generator agent to reuse existing components from your codebase instead of generating new ones from scratch. Run `/compose-design scan-components` to auto-populate this config.
...
│   └── scan-components.md       # Component library scanner
├── agents/
